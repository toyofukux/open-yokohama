...
2026-09-07 統合後の区別：旧統計3記事は撤去済みで、33主張の署名は履歴として保管する。
現行7政策記事の公開は `docs/content-review/publication.json` と `scripts/validate-editorial.ts` が検査する。
旧記事の署名や独立レビューを、新記事の事実確認済みの印として表示しない。
通常の `factcheck:check` は歴史的署名と現行原稿の公開契約、`test:factcheck-gates` は両者の改変拒否を確認する。
原本込みの旧記事監査は `factcheck:archive-originals`、その原本改変の拒否は `test:factcheck-archive-originals`。
固定HTMLは手元の保管版が必要で、上流の変更後に同じ版を新規取得できるとは限らない。
以下は元の統計記事パイプラインの設計・履歴。旧記事を再公開する指示ではない。
