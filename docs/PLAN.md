...
2026-09-07: リデザインの目的・情報構成・工程・判断は[統合文書](REDESIGN-PLAN.md)を正本とします。
争点→判断条件→具体的な問い→根拠へ逆算する方針を維持します。
初期試作3案は内容不足のため採用比較から外し、技術資産として保持します。
統合文書第6.2節に候補比較とブリーフを追記し、[中学校給食費の内容原稿](content-review/school-lunch/DRAFT.md)と根拠対応・自己点検を作成しました。
ユーザーが原稿の大枠を採用し、給食費ページの初版とホーム・一覧・検索の導線を実装しました。詳細は統合文書第6.3節です。
2026-09-07、ユーザーから本番公開と主要論点の制作・公開を委任されました。給食に市政の信頼・財政・GREEN×EXPO・交通・保育・学校体育館の空調を加えた7論点と、2026年市長選の入口を初版の対象とします。
次は公開内容の訂正・未確定費用の調査、条件をそろえた政策間の配分比較、投票を考える基礎記事の順です。公約公表に応じた比較・不足論点の追加、選挙後の約束の追跡も統合文書第6.4節に整理しました。基礎記事は将来計画への追加で、本文の制作は今回行いません。
独立した事実確認・人間の利用者検証は未実施です。公開許可は、全サイトの構成採用や理解の検証済みを意味しません。
内容が成立した後に画面の解決案を比較し、人間の初期検証を経て詳細設計を固定します。
2026-09-07: 横浜市DWHの整備と記事からの参照を目的に追加しました。
人口分野の共通台帳・不変版・19件の記事参照・更新影響・静的API・内部MCPを実装しました。
[実装内容と残る点](DWH-IMPLEMENTATION.md)を踏まえ、文書ファクトの共通移行・国内都市比較・公共的論点・財政へ展開します。

同日の追加指示により、MCPの外部公開は停止しました。APIキー・クラウド認証があっても再公開せず、今後のMCP検証はローカルに限定します。
以下の過去の公開計画・記録より、この停止方針を優先します。具体例は[DWHの全体説明](DWH-EXAMPLES.md)を参照してください。

2026-09-06: 市民向けの独自デザインシステムと既存テーマを統合する[リデザイン計画](REDESIGN-PLAN.md)を作成しました。
当時の計画案です。その後の7政策記事・地図・ホーム実装と未実施の人間検証は、上記の統合文書とSTATUSを参照してください。

## 成果の定義

市民がスマートフォンから、ログインせず気になった争点や暮らしの問いを開き、判断条件・具体的な答え・根拠を確認できます。
自分の区や他区との比較は、その問いに必要な材料として利用できます。
文章は「まず要点→詳しく→原典」の順にします。同じ事実を全員に提供し、読み方を本人が選びます。
リデザインの主評価は、任意の評価参加者における検証付き問い前進率（QPR）と重大誤解の確認です。定義・未計測状態は統合文書に従います。
実利用の成功は公開だけでは判定しません。初期利用者10人中8人が「自分の区を探す・他区と比較する・原典を開く」を各2分以内に完了、重大な誤解0件を操作性の公開拡大目標とします。問いの理解や運営の成立は別に確認します。
...
以下は公共情報基盤の段階計画です。現在のリデザインは統合文書のG0〜G5を適用し、一問の理解・維持を確認してから内容を拡充します。
...
2026-09-06時点の実装順案は[根拠・訂正を中心とする実装計画](BACKLOG.md#根拠訂正を中心とする実装計画最新案)に残します。最新の順序は[BACKLOGのDWH追記](BACKLOG.md#dwh-next)を参照してください。国内比較・公共的論点・財政の前に、既存人口データの共通化と記事参照を実証します。
